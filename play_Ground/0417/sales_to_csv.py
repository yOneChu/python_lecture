...
- productNo 하나당 CSV 파일 하나를 만든다. 파일명은 '<호기번호>_sales_<YYYYMMDD>.csv'.
- 파일은 스크립트를 실행한 폴더(현재 작업 디렉터리)에 생성된다.
- CSV 는 가로가 아니라 세로(key-value) 형식으로 쓴다.
  첫 줄에 'key,value' 헤더를 두고, 응답의 필드명을 key 열에, 값을 value 열에 한 줄씩 적는다.
  레코드가 여러 건이면 빈 줄로 구분하고 '[n/전체]' 표시를 붙인다.
...
# 설정
# ----------------------------------------------------------------------------
BASE_URL = "https://vault-in.hdel.co.kr:8070/api/findElvSearch"
API_KEY = "subae"
...
# 요청 타임아웃 (연결 대기, 응답 대기) 초.
# BOM 조회는 서버에서 2분 안팎이 걸리므로 읽기 대기를 넉넉히 준다.
# 하나의 숫자를 주면 연결/읽기에 같은 값이 적용된다.
TIMEOUT = (10, 300)
...
def fetch_bom(session, product_no):
    ...
    for attempt in range(1, MAX_RETRY + 1):
        try:
            started = time.time()  # 응답까지 얼마나 걸렸는지 보여주기 위한 시작 시각
            response = session.get(
                BASE_URL,
            ...
            # 서버 조회가 2분 안팎 걸리므로, 멈춘 게 아니라는 걸 알 수 있게 시간을 찍는다.
            print(f"  -> 응답 수신 ({time.time() - started:.1f}초)")
            return records  # 성공 시 BOM 레코드 목록 반환
        except (requests.RequestException, ValueError) as exc:
            last_error = exc
            if attempt < MAX_RETRY:
...
def write_csv(product_no, records, today):
    """레코드를 '<호기번호>_sales_<YYYYMMDD>.csv' 로 저장하고 경로를 반환한다.

    가로 표가 아니라 세로(key-value) 형식으로 쓴다.
        key,value
        productNo,N27748L02
        partNo,ABC-123
    """
    total = len(records)
    # 파일명: '<호기번호>_sales_<오늘날짜>.csv'  (예: N27748L02_sales_20260824.csv)
    out_path = OUTPUT_DIR / f"{safe_filename(product_no)}_sales_{today}.csv"
    # utf-8-sig: 엑셀에서 한글이 깨지지 않도록 BOM 을 붙인다.
    with open(out_path, "w", newline="", encoding="utf-8-sig") as fp:
        writer = csv.writer(fp)
        for index, record in enumerate(records, start=1):
            # dict 가 아닌 값은 'VALUE' 라는 단일 키로 감싼다.
            if not isinstance(record, dict):
                record = {"VALUE": record}
            # 레코드가 여러 건이면 빈 줄 + 구분선으로 나눠서 눈에 띄게 한다.
            if total > 1:
                if index > 1:
                    writer.writerow([])
                writer.writerow([f"[{index}/{total}]", ""])
            # 각 레코드마다 key/value 헤더를 한 번 적는다.
            writer.writerow(["key", "value"])
            # 응답에 담긴 순서대로 한 줄에 하나씩 key,value 를 쓴다.
            for field, value in record.items():
                writer.writerow([field, flatten_value(value)])
    return out_path
